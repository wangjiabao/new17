...
// UpdateUserThree .
func (ui *UserInfoRepo) UpdateUserThree(ctx context.Context, userId int64, amount uint64, one, two, three string, four int64) error {
	...
	res = ui.data.DB(ctx).Table("user_balance").
		Where("user_id=?", userId).
		Updates(map[string]interface{}{"balance_usdt_float": 0})
	if res.Error != nil {
		return errors.New(500, "UPDATE_USER_ERROR", "用户信息修改失败")
	}
